...
to analyze the trust a user of such service must place in their
...
results appear in Tables~\ref{tab:trust:app:cap}
and~\ref{tab:trust:app:threats}.
...
effectively have full control over any data end-user stores atop
EC2. Amazon could (theoretically) read all system memory, all system
disk, and even alter the behoove of the processor in such a system to
control the execution flow of any software running on EC2. EC2 thus
requires the end user to grant Amazon full capabilities to store,
read, modify, or meta-analyses any the data stored or processed via an
EC2-backed VM.

The profile of Amazon's proclivity for trust violation follow that of
other service-oriented cloud operators such as Dropbox, Tresorit,
etc. Amazon charges user for the right to use EC2, and faces
...
disincentivized from committing unintentional insider or outsider
violations.
